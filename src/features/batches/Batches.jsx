...
const Batches = () => {
  ...
  if (isLoading) return <p className="text-amber-500 text-center">Loading Batches...</p>;
  if (isError) return <p className="text-red-500 text-center">Failed to load batches.</p>;
  ...
  return (
    <div className="mx-auto p-6 max-w-5xl">
      ...
      <div className="mb-8 text-center">
        <h1 className="mb-2 font-extrabold text-gray-800 text-4xl">All Batches</h1>
          ...
          {batches.map((batch) => (
            <tr key={batch.id} className="even:bg-amber-50 text-center">
              ...
              <td className="space-x-2 px-4 py-2 border">
                <button
                  onClick={() => navigate(`/edit/${batch.id}`)}
