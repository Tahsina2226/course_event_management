...
const EditBatch = () => {
  ...
  const [editData, setEditData] = useState({ name: '', department: '', semester: '' });
  ...
  if (isLoading) return <p className="mt-6 text-gray-500 text-center">Loading batch data...</p>;
  if (isError) return <p className="mt-6 text-red-400 text-center">Failed to load batch data.</p>;
  if (!batch) return <p className="mt-6 text-gray-600 text-center">Batch not found</p>;
  ...
  const handleUpdate = async (e) => {
    ...
    const result = await Swal.fire({
      ...
      customClass: {
        ...
        cancelButton: 'py-2 px-6 rounded-md bg-gray-300 hover:bg-gray-400 font-semibold tracking-wide',
      }
    });
    ...
    try {
      await updateBatch({ id, data: editData }).unwrap();
      toast.success('Batch updated successfully!', { style: { background: '#d1fae5', color: '#065f46' } });
      navigate('/batches');
    } catch (err) {
      toast.error('Update failed!', { style: { background: '#fee2e2', color: '#991b1b' } });
    }
  };
  ...
  return (
    <div className="bg-white shadow-lg hover:shadow-xl mx-auto mt-14 p-10 border border-gray-200 rounded-2xl max-w-lg transition-shadow duration-300">
      ...
      <form onSubmit={handleUpdate} className="space-y-8">
        <div>
          <label className="block mb-3 font-semibold text-gray-700 text-lg">Name</label>
          <input
            type="text"
            value={editData.name}
        ...
        <div>
          <label className="block mb-3 font-semibold text-gray-700 text-lg">Department</label>
          <input
            type="text"
        ...
        <div>
          <label className="block mb-3 font-semibold text-gray-700 text-lg">Semester</label>
          <input
            type="text"
